...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    file_put_contents("php://stderr", print_r($data, true)); // Will log to PHP error log

    // Make sure required POST data exists
    if (isset($data['user_id'], $data['item_id'], $data['item_image'], $data['item_name'], $data['item_price'], $data['quantity'], $data['special_request'])) {
        ...
        $stmt->bind_param("iiidsss", $userId, $itemId, $quantity, $itemPrice, $state, $itemName, $itemImage);

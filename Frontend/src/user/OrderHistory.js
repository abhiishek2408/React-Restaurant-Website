...
const OrderHistory = () => {
  ...
  useEffect(() => {
    if (!effectiveUser || !effectiveUser.user_id) return;

    fetch('http://localhost/onlinerestro/backend/orderHistory.php', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      credentials: 'include',
      body: JSON.stringify({ user_id: effectiveUser.user_id }),
    })
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
      ...
      {orders.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          {orders.map((order) => (
            ...
            >
              <img src={order.item_image || 'https://via.placeholder.com/100'} alt={order.item_name}
                style={{
                  width: '100px',
                  height: '100px',
                  objectFit: 'cover',
